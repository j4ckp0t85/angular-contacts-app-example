...
export class ContactsEffects {
  ...
  loadAll$ = createEffect(() => this.actions$.pipe(
    ...
    // startWith(loadAll()), QUESTO CONSENTIREBBE DI EVITARE L'AZIONE DI DISPATCH LOADALL - SI RICHIAMA DA SOLO INIZIALMENTE QUANDO VIENE DEFINITO L'EFFETTO
  ...
  updateOk$ = createEffect(() => this.actions$.pipe(
    ofType(updateSuccess),
    pluck('contact'),
    exhaustMap(contact => this.router.navigate(['contacts', contact.id]))
  ),
    { dispatch: false }
  );

  removeOk$ = createEffect(() => this.actions$.pipe(
    ofType(removeSuccess),
    pluck('id'),
    exhaustMap(id => this.router.navigateByUrl('contacts'))
  ),
    { dispatch: false }
  );
